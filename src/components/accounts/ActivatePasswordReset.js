...
export default function ActivatePasswordReset(props) {
    ...
    async function activatePasswordReset(code) {

        let status = document.getElementById('activated');
        
        await axios({
            method: 'put',
            url: `${serverEndPoint}activate-password-reset/${code}`,
        })
        .then(response => {
            
            if(response.data.data.type == "success" || response.data.data.type == "done") {
                status.innerHTML = `<p style='color:green'>Done. Please wait ... </p>`;
                setTimeout(function() {
                    window.location = `/reset-password/${values.code}`;
                }, 2000);
            } else {
    ...
    useEffect( () => {
        activatePasswordReset(values.code);
